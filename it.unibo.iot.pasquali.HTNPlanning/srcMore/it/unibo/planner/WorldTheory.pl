%==============================================
% WorldTheory.pl for actor planner
%==============================================
/*
For a QActor as a singleton statically degined in the model
*/
myname(qatuplanner).	%%old version (deprecated)
actorobj(qatuplanner).	%% see registerActorInProlog18 in QActor
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor planner
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of planner" ). 
...
%-------------------------------------------------
%  User static rules about planner
%------------------------------------------------- 
/*
------------------------------------------------------------------------
...
actorOp( Op )   :- % actorPrintln( actorOp( Op  ) ),
				   java_catch(
						qatuplanner <- Op returns R, 	%% R unbound if void
...
%% setActionResult : qatuplanner register (in Java) the result under name 'actoropresult'
...
cvtToString( V , "void" ):-
	unbound(V),!,
	qatuplanner <- setActionResult( void ).
cvtToString( V , S ):-
	number(V),!,
	setResult(V),
	text_term( S, V ).
