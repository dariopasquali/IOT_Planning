...
%==============================================
test :-
	stdout <- println(  "hello from world theory in it.unibo.robotUsage.avatar" ).
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor arbiter
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of arbiter" ). 
...
actorPrintln( X ):- arbiter  <- println( X ).

%-------------------------------------------------
%  User static rules about arbiter
%------------------------------------------------- 
/*
------------------------------------------------------------------------
execActor( testForProlog( X)):- 
	java_catch(
		 arbiter <- testForProlo( X) ,
		[('java.lang.Exception'(Cause, Msg, StackTrace),output(Msg))],
		output(done)
	).
execActor( ANY ):- arbiter  <- println( execActor(failure,ANY) ).
------------------------------------------------------------------------
*/
execActor( Op ) :- output( Op ), arbiter <- Op, !.
