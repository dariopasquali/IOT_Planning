...
actorOp( Op )   :- % actorPrintln( actorOp( Op  ) ),
				   java_catch(
						qaturobot <- Op returns R, 	%% R unbound if void
...
%% setActionResult : qaturobot register (in Java) the result under name 'actoropresult'
...
cvtToString( V , "void" ):-
	unbound(V),!,
	qaturobot <- setActionResult( void ).
cvtToString( V , S ):-
	number(V),!,
	setResult(V),
	text_term( S, V ).
