...
actorOp( Op )   :- % actorPrintln( actorOp( Op  ) ),
				   java_catch(
						qatuconsole <- Op returns R, 	%% R unbound if void
...
%% setActionResult : qatuconsole register (in Java) the result under name 'actoropresult'
...
cvtToString( V , "void" ):-
	unbound(V),!,
	qatuconsole <- setActionResult( void ).
cvtToString( V , S ):-
	number(V),!,
	setResult(V),
	text_term( S, V ).
