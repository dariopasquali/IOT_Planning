...
public class Check extends ConditionalAction implements Serializable{
	...
	private void init(boolean clone){
		...
		this.addEffect(f);		
		if(!clone) labels.add(new ConditionalLabel(f.toString()+"-"+label, 1, f, null, label));
		...
		f.addParam(to.toString());
		this.addEffect(f);
		if(!clone) labels.add(new ConditionalLabel(f.toString()+"-"+label, 2, f, null, label));
	...
	public String getHash(){
		...
		String s = "check"+from.toString()+to.toString();
